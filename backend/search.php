...
$query = "SELECT * FROM `expenses` WHERE `purpose` LIKE '$data%' && `user_id`='$u_id' ORDER BY `date` DESC";
...
if($count > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        $output .= "<tr>
            ...
            $select1 = "SELECT SUM(amount) FROM `expenses` WHERE `purpose` LIKE '%$data%' && `user_id`='$u_id'";
